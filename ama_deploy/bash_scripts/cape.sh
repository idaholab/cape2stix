...
RRM5efxf1F532A==
=Cc71
-----END PGP PUBLIC KEY BLOCK-----" | sudo gpg --dearmor -o /etc/apt/keyrings/mongo.gpg --yes
...
sed -i 's/curl -fsSL/curl -fsSL --insecure/g' /tmp/cape2.sh
/tmp/cape2.sh all cape | tee /home/vagrant/cape.log
reboot now
